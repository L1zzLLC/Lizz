...
object LocalSettingsCommand : Command("localsettings", "localsetting", "localconfig") {
    ...
    override fun execute(args: Array<String>) {
        val usedAlias = args.getOrNull(0)?.lowercase() ?: "localsettings"
    ...
    private suspend fun loadSettings(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                chat("§cFailed to load settings: ${e.message}")
            }
        }
    }
    ...
    private suspend fun saveSettings(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                // NOTE: change here: default now includes binds as well
                val values = all || default || "values" in option
                val binds = all || default || "binds" in option    // <-- default includes binds now
                ...
                addNotification(Notification.informative("Local Settings Command", "Saved Settings"))
                playEdit()
    ...
    private suspend fun deleteSettings(args: Array<String>) {
        withContext(Dispatchers.IO) {
            ...
            if (settingsFile.delete()) {
                ...
                chat("§cFailed to delete settings file.")
            }
        }
    }
