...
object Velocity : Module("Velocity", Category.COMBAT) {
    ...
    private val mode by choices(
        "Mode", arrayOf(
            ...
            "Click", "BlocksMC", "GrimCombat", "Polar", "Buffer"
        ), "Simple"
    )
        ...
        if (mode == "Buffer") {
            ...
            while (iterator.hasNext()) {
                ...
                if (bufferedPacket.remainingTicks <= 0) {
                    // Apply the buffered velocity packet
...
//        ) {
        ...
        if ((packet is S12PacketEntityVelocity && thePlayer.entityId == packet.entityID)
            ...
            when (mode.lowercase()) {
                ...
                "buffer" -> {
                    // Buffer mode: cancel the original packet and buffer it for delayed processing
                    event.cancelEvent()
    ...
    /**
     * Data class for buffered packets in Buffer mode
     */
