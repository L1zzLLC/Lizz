    ...
    var state = defaultState
        set(value) {
            ...
            if (!isStarting) {
                try {
                    if (value) {
                        JLayerUtils.playMP3("/assets/minecraft/${CLIENT_NAME.lowercase()}/sounds/Enable/Sigma5.mp3")
                        WaterMark.showToggleNotification("Module Toggled", "$name has been §aEnabled§f!", true,930,name)
                    }else{
                        JLayerUtils.playMP3("/assets/minecraft/${CLIENT_NAME.lowercase()}/sounds/Disable/Sigma5.mp3")
                        WaterMark.showToggleNotification("Module Toggled", "$name has been §cDisabled§f!", false,930,name)
                    }
                    val state = if (value) {
