...
import net.ccbluex.liquidbounce.utils.extras.sendOffHandUseItem.sendOffHandUseItem
...
object KillAura : Module("KillAura", Category.COMBAT, Keyboard.KEY_R) {
    ...
    // CPS - Attack speed
    private val cps by intRange("CPS", 7..9, 1..50) { !simulateCooldown }
    ...
    private val noFucker by boolean("NoFucker", false)

    // Range
    // TODO: Make block range independent from attack range
    val range: Float by float("Range", 3.45f, 1f..8f).onChanged {
        blockRange = blockRange.coerceAtMost(it)
    }
    private val scanRange by float("ScanRange", 1f, 0f..10f)
    private val throughWallsRange by float("ThroughWallsRange", 0f, 0f..8f)
    ...
    // Delay
    private val switchDelay by int("SwitchDelay", 30, 1..1000) { targetMode == "Switch" }
    ...
    // AutoBlock
    val autoBlock by choices("AutoBlock", arrayOf("Off", "Packet", "Fake", "BlocksMC","QuickMarco"), "Packet")
    ...
    // Prediction
    private val predictClientMovement by int("PredictClientMovement", 0, 0..5)
    ...
    private fun shouldPrioritize(): Boolean = when {
        ...
        noFucker && Fucker.handleEvents() -> true
    ...
    private fun drawCustomCircle() {
        ...
        for (i in 0..360 step 61 - circleAccuracy) { // You can change circle accuracy  (60 - accuracy)
            GL11.glVertex2f(
        ...
        GL11.glVertex2f(
            cos(360 * Math.PI / 180.0).toFloat() * range,
            (sin(360 * Math.PI / 180.0).toFloat() * range)
        )
