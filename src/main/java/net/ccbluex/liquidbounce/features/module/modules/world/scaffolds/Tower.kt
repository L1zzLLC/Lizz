...
object Tower : Configurable("Tower"), MinecraftInstance, Listenable {
    ...
    val onMove = handler<MoveEvent> { event ->
        if (isTowering && towerModeValues.get() == "BlocksMC"){
            if (net.ccbluex.liquidbounce.utils.MinecraftInstance.mc.thePlayer.isPotionActive(Potion.moveSpeed)) {
    ...
    private fun move() {
        ...
        when (towerModeValues.get().lowercase()) {
            "blocksmc" -> {
                val player = Companion.mc.thePlayer ?: return
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        if (packet is C08PacketPlayerBlockPlacement) {
            ...
            ReflectionUtil.setFieldValue(packet, "stack", Companion.mc.thePlayer.inventory.mainInventory[Companion.mc.thePlayer.inventory.currentItem])
            ...
            if (towerModeValues.equals("BlocksMC") && isTowering) {
                if (Companion.mc.thePlayer.motionY > -0.0784000015258789) {
                    if (packet.position.equals(
                            BlockPos(
                                Companion.mc.thePlayer.posX,
                                Companion.mc.thePlayer.posY - 1.4,
                                Companion.mc.thePlayer.posZ
                            )
                        )
                    ) {
                        Companion.mc.thePlayer.motionY = -0.0784000015258789
                    }
                }
            }
        }
