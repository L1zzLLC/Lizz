...
object ChestAura : Module("ChestAura", Category.WORLD) {
    ...
    val onRender3D = handler<Render3DEvent> { event ->
        // 只在有已开启的箱子时渲染
        if (clickedTileEntities.isEmpty()) return@handler

        // 设置纯红色
        val redColor = Color(255, 0, 0)

        // 创建一个临时集合来存储需要移除的已破坏箱子
        val toRemove = mutableSetOf<TileEntity>()

        for (tileEntity in clickedTileEntities) {
            // 只渲染普通箱子，忽略末影箱子
            if (chest && tileEntity is TileEntityChest) {
                // 检查箱子是否仍然存在（没有被破坏）
                val block = tileEntity.pos.block
                if (block is BlockChest) {
                    // 使用StorageESP中OtherBox的样式渲染
                    drawBlockBox(tileEntity.pos, redColor, false)
                } else {
                    // 箱子已被破坏，标记为需要移除
        ...
        if (toRemove.isNotEmpty()) {
            clickedTileEntities.removeAll(toRemove)
        }
    }
