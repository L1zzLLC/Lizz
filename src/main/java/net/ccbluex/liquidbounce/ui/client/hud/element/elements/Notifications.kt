            ...
            "Classic" -> {
                ...
                val (backgroundColor, borderColor, textColor) = when (severityType) {
                    Notifications.SeverityType.SUCCESS -> Triple(
                        Color(28, 148, 97).withAlpha(element.color.alpha.coerceAtLeast(180)),  // 柔和的绿色
                        Color(46, 170, 80).withAlpha(230),  // 深一点的绿色边框
                        Color.WHITE
                    )
                    Notifications.SeverityType.RED_SUCCESS -> Triple(
                        Color(137, 39, 39).withAlpha(element.color.alpha.coerceAtLeast(180)),  // 柔和的红色
                        Color(229, 57, 53).withAlpha(230),  // 深一点的红色边框
                        Color.WHITE
                    )
                    Notifications.SeverityType.INFO -> Triple(
                        Color(52, 152, 219).withAlpha(element.color.alpha.coerceAtLeast(180)),  // 柔和的蓝色
                        Color(41, 128, 185).withAlpha(230),  // 深一点的蓝色边框
                        Color.WHITE
                    )
                    Notifications.SeverityType.WARNING -> Triple(
                        Color(255, 193, 7).withAlpha(element.color.alpha.coerceAtLeast(180)),  // 柔和的黄色
                        Color(245, 166, 35).withAlpha(230),  // 深一点的黄色边框
                        Color(33, 33, 33)  // 深色文本以提高对比度
                    )
                    Notifications.SeverityType.ERROR -> Triple(
                        Color(239, 83, 80).withAlpha(element.color.alpha.coerceAtLeast(180)),  // 柔和的错误红色
                        Color(222, 50, 50).withAlpha(230),  // 深一点的错误红色边框
                        Color.WHITE
                    )
                }
                ...
                if (element.renderBorder) {
                    drawRoundedBorder(
                        ...
                        -y,
                        element.borderWidth,
                        borderColor.rgb, // 使用状态对应的边框色
                        element.roundRadius
                    )
                }
                val nearTopSpot = -y - MAX_HEIGHT + 10
                // 修改为右对齐的文本绘制 - 调整x坐标以实现右对齐
