...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F, side: Side = Side.default()) : Element(
    ...
    private var onScaffold2 by boolean("Scaffold2Only", false) // 新增的 Scaffold2Only 选项
    ...
    override fun drawElement(): Border {
        ...
        if (shadowK) {
            ...
            // 绘制阴影
            if ((!onScaffold && !onScaffold2) ||
                (onScaffold && Scaffold.handleEvents()) ||
                (onScaffold2 && Scaffold2.handleEvents()) ||
                mc.currentScreen is GuiHudDesigner) {
                drawGlow(bgX.toFloat(), bgY.toFloat(), bgWidth.toFloat(), bgHeight.toFloat(),
        ...
        assumeNonVolatile {
            // 检查是否应该渲染文本元素
            val shouldRenderText = (!onScaffold && !onScaffold2) ||
                    (onScaffold && Scaffold.handleEvents()) ||
                    (onScaffold2 && Scaffold2.handleEvents()) ||
                    mc.currentScreen is GuiHudDesigner
